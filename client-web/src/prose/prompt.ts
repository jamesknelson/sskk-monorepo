...
function reportInvalid(dom: InputElement, message: string) {
...
export abstract class Field<Options extends FieldOptions = FieldOptions> {
  ...
  abstract render(): InputElement
  // Render the field to the DOM. Should be implemented by all subclasses.

  // :: (dom.Node) → any
  // Read the field's value from its DOM node.
  read(dom: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement) {
    return dom.value
  }

  // :: (any) → ?string
  // A field-type-specific validation function.
  validateType(_value: any): string | void {}
...
export class SelectField extends Field<SelectFieldOptions> {
  render() {
