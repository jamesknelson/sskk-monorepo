...
export function CodeBlock({
  ...
  const languageClassIndex = classes.findIndex(
    (x) => x.slice(0, languageClassPrefix.length) === languageClassPrefix,
  )
  const language =
    languageProp ??
    (languageClassIndex === -1
      ? 'text'
      : classes[languageClassIndex].slice(languageClassPrefix.length))
  ...
  return (
    <pre
      ...
      className={
        languageClassIndex !== -1
          ? className
          : [className, 'language-' + language].join(' ')
      }
      css={[
        blockVerticalGutter,
